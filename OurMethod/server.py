...
import random
from Models import Mnist_2NN, Mnist_CNN, Mnist_CNN2
from client import *
...
parser.add_argument('-repoi', '--repoisonalgorithms', type=str, default='fedavg', help='set the repoison algorithms')
    ...
    for globalEpoch in range(args['num_comm']):
        ...
        for index, cluster in tqdm(enumerate(clustergroup), total=len(clustergroup)):
            ...
            for data, label in testDataLoader:
                ...
                num += 1
            print("\n" + '测试训练完成分簇accuracy: {}'.format(sum_accu / num))
            # test_txt.write('测试分簇本地训练的accuracy: ' + str(float(sum_accu / num)) + "  ")
        ...
        elif args['repoisonalgorithms'] == 'fedavg':
            global_parameters = fedavg(blockchain)
