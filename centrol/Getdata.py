...
class GetDataSet(object):
    ...
    def mnistDataSetConstruct(self):
        ...
        if self.isIID:
            ...
            train_label = torch.argmax(torch.tensor(self.train_label), dim=1)
            self.train_dataset = TensorDataset(torch.tensor(self.train_data), torch.tensor(train_label))
        else:
            '''
            ...
            self.train_data = train_images[order]
            self.train_label = train_labels[order]
            train_label = np.argmax(self.train_label, axis=1)
            self.train_dataset = TensorDataset(torch.tensor(self.train_data), torch.tensor(train_label))
        ...
        test_label = torch.argmax(torch.tensor(self.test_label), dim=1)
        self.test_data_loader = DataLoader(TensorDataset(self.test_data, test_label), batch_size=100, shuffle=False)
